...
function App() {
	...
	useEffect(() => {
		if (leftSelect && rightSelect) {
			const result = (Number(leftInput) * leftSelect.rate) / rightSelect.rate;
			setRightInput(Number(result.toFixed(0)));
		}
	}, [leftInput]);

	useEffect(() => {
		if (leftSelect && rightSelect) {
			const result = (Number(rightInput) * rightSelect.rate) / leftSelect.rate;
			setLeftInput(Number(result.toFixed(0)));
		}
	}, [rightInput]);
